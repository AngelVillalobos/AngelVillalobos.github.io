...
Pelota.prototype.sense=function(environment)
{
  ...
  this.sensor.set(this.position,new THREE.Vector(-1,0,0));
  var obstaculo2=this.sensor.intersectObjects(environment.children,true);
  if((obstaculo1.length>0 && (obstaculo1.distance<=this.radius))||(obstaculo2.length>0 && (obstaculo2.distance<=this.radius)))
    this.colision=1;
  else
    this.colision=0;
}
